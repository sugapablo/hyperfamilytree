...
class tree_ancs {
	...
	function parentNode($id,$first_name,$last_name,$maiden_name,$gender,$level) {
		if($level == 1)
			$parent = "";
		elseif($level == 2)
			$parent = "grand";
	...
	function get_parents($id,$level) {
		...
		while(list($fparent,$ffirst_name,$flast_name,$fmaiden_name,$fgender,$mparent,$mfirst_name,$mlast_name,$mmaiden_name,$mgender) = mysql_fetch_row($res)) {
			...
			if($fparent) {
				...
				$string.= $this->parentNode($fparent,$ffirst_name,$flast_name,$fmaiden_name,$fgender,$level);
				$go++;
			}
			...
			if($mparent) {
				...
				$string.= $this->parentNode($mparent,$mfirst_name,$mlast_name,$mmaiden_name,$mgender,$level);
				$go++;
			}
		}
	...
	function show($id,$tree = 0) {
		...
		if(!$tree) {
			$string.= "		<div class=\"tree\">
							<p class=\"tree\"><a href=\"person.php?id=".$id."\">".$first_name."&nbsp;".$maiden_name.$last_name."</a></p>";
			$string.= "		</div>";
			$string.= "	</td>
					</tr>
		      	     </table>";
